            ...
            <ul className="list-reset text-white p-0">
                {state.todos.map(todo => (
                    <li key={todo.id} className="flex items-center bg-orange-dark border-black border-dashed border-2 my-2 py-4">
                        <span
                            onDoubleClick={async () => {
                                const response = await axios.patch(
                                    `https://hooks-api-master.opps4lrng.now.sh/todos/${todo.id}`,
                                    {
                        ...
                        <button
                            onClick={async () => {
                                await axios.patch(`https://hooks-api-master.opps4lrng.now.sh/todos/${todo.id}`)
                        ...
                        <button
                            onClick={async () => {
                                axios.delete(`https://hooks-api-master.opps4lrng.now.sh/todos/${todo.id}`)
